  ...
  $.ajax({
    ...
    success: function (result) {
      ...
      for (var i = 0; i < result.length; i++) {
        ...
        let botonDetalleUsuario = document.createElement("button");
        botonDetalleUsuario.value = result[i]["id_usuario"];
        botonDetalleUsuario.innerHTML = "Detalle";
        botonDetalleUsuario.onclick = function (e) {
          $('#EditarModal').modal('show');
          consultarUsuarioID(this.value);
        }
        botonEditarUsuario.className = "btn btn-warning detalle_usuario";
